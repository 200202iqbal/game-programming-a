...
def add_bomb():
    while True:
        pos = (random.randint(0,W-1),random.randint(0,H-1))
        if pos in BOMBS or pos in SNAKE:
            continue
        BOMBS.append(pos)
        break
...
def paint(message):
    ...
    for bomb in BOMBS:
        pygame.draw.ellipse(SURFACE,(255,0,0),Rect(bomb[0] * 30,bomb[1] *30,30,30))
...
def main():
    ...
    for _ in range(10):
        add_food()
    for _ in range(int(NUM_OF_BOMBS)):    
        add_bomb()
    ...
    while True:
        ...
        if not game_over:
            ...
            if head in SNAKE or head[0] < 0 or head[0]>= W or head[1] < 0 or head[1] >= H or head in BOMBS: 
